...
from settings import get_setting, set_setting, save_settings
...
class ControllerModeManager:
    ...
    def __init__(self):
        ...
        self.bumper_hold_time = 0.5  # seconds to hold bumper to change mode (500ms)
        self.left_bumper_pressed_time = None
        self.right_bumper_pressed_time = None
    ...
    def _save_settings(self):
        ...
        try:
            from settings import load_settings, save_settings as save_settings_func
            settings = load_settings()
    ...
    def handle_trigger_press(self, is_left: bool, pressed: bool):
        ...
        Hold trigger for 500ms to cycle modes
        """
        current_time = time.time()

        if is_left:
            if pressed:
                if self.left_bumper_pressed_time is None:
                    self.left_bumper_pressed_time = current_time
                elif self.left_bumper_pressed_time > 0:
                    # Check if held long enough
                    hold_duration = current_time - self.left_bumper_pressed_time
                    if hold_duration >= self.bumper_hold_time:
                        self.cycle_mode()
                        self.left_bumper_pressed_time = -1  # Prevent re-trigger
            else:
                self.left_bumper_pressed_time = None
        else:  # Right trigger
            if pressed:
                if self.right_bumper_pressed_time is None:
                    self.right_bumper_pressed_time = current_time
                elif self.right_bumper_pressed_time > 0:
                    # Check if held long enough
                    hold_duration = current_time - self.right_bumper_pressed_time
                    if hold_duration >= self.bumper_hold_time:
                        self.cycle_mode()
                        self.right_bumper_pressed_time = -1  # Prevent re-trigger
            else:
                self.right_bumper_pressed_time = None
