...
from controller_vibrations import vibration_controller
from translation import _
from sound import play_sound
from accessible_output3 import outputs
from stereo_speech import get_stereo_speech
from settings import get_setting
from controller_modes import get_mode_manager, ControllerMode
            ...
            if self.parent_window:
                ...
                for i in range(joystick_count):
                    try:
                        ...
                        # Try multiple axis configurations for different controller types
                        num_axes = controller.get_numaxes()

                        # Initialize trigger detection on first run
                        if not hasattr(self, '_trigger_axes_detected'):
                            self._trigger_axes_detected = False
                            # Xbox controller: LT=axis 5, RT=axis 4 (verified from pygame docs)
                            self._left_trigger_axis = 5   # Xbox Left Trigger
                            self._right_trigger_axis = 4  # Xbox Right Trigger
                            print(f"[CONTROLLER] Controller has {num_axes} axes (LT=axis {self._left_trigger_axis}, RT=axis {self._right_trigger_axis})")

                        if num_axes >= 4:  # Need at least 4 axes
                            # Get trigger values based on detected configuration
                            left_trigger = 0.0
                            right_trigger = 0.0

                            # Try different axis configurations
                            if num_axes > self._right_trigger_axis:
                                # Standard Xbox layout (axis 2 and 5)
                                left_trigger = controller.get_axis(self._left_trigger_axis)
                                right_trigger = controller.get_axis(self._right_trigger_axis)
                            elif num_axes >= 4:
                                # Alternative layout - use last 2 axes
                                left_trigger = controller.get_axis(num_axes - 2)
                                right_trigger = controller.get_axis(num_axes - 1)

                            # Debug: Print trigger values when they change
                            if not hasattr(self, '_last_trigger_values'):
                                self._last_trigger_values = (-1.0, -1.0)

                            if abs(left_trigger - self._last_trigger_values[0]) > 0.3 or \
                               abs(right_trigger - self._last_trigger_values[1]) > 0.3:
                                print(f"[TRIGGER] L(axis {self._left_trigger_axis})={left_trigger:+.2f}, R(axis {self._right_trigger_axis})={right_trigger:+.2f}")
                                self._last_trigger_values = (left_trigger, right_trigger)

                            # Handle trigger press for mode switching
                            # Xbox triggers: -1.0 (unpressed) to +1.0 (fully pressed)
                            # BUT: After first use, they may stay at +1.0 when unpressed!
                            # Solution: Check if significantly moved from rest position

                            # Calculate if trigger is "pressed" (moved significantly from -1.0)
                            # We use -0.3 as threshold (trigger needs to be pressed at least 35%)
                            left_pressed = left_trigger > -0.3
                            right_pressed = right_trigger > -0.3
