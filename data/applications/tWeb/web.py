...
def play_sound(sound_file):
    ...
    if not os.path.exists(sound_path):
        print(f"Nie znaleziono pliku dźwiękowego: {sound_path}")
        return
    try:
...
class DownloadsDialog(wx.Dialog):
    def __init__(self, parent, downloads):
        super(DownloadsDialog, self).__init__(parent, title="Pobrane pliki")
        self.parent = parent
        ...
        info_text = wx.StaticText(panel, label="Lista plików pobranych przez tBrowser:")
...
class BrowserFrame(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.back_button = wx.Button(self.toolbar, label="Wstecz")
        self.forward_button = wx.Button(self.toolbar, label="Dalej")
        self.refresh_button = wx.Button(self.toolbar, label="Odśwież")

        self.back_button.SetToolTip("Przycisk Wstecz")
        self.forward_button.SetToolTip("Przycisk Dalej")
        self.refresh_button.SetToolTip("Przycisk Odśwież")

        self.address = wx.TextCtrl(self.toolbar, style=wx.TE_PROCESS_ENTER)
        self.address.SetHint("Wpisz adres lub wyszukaj w Google")
    ...
    def CreateMenuBar(self):
        ...
        settings_item = app_menu.Append(wx.ID_ANY, "Ustawienia...")
        menubar.Append(app_menu, "Aplikacja")
    ...
    def OnBack(self, event):
        if isinstance(self.browser, wx.html2.WebView):
            ...
            wx.MessageBox("Nawigacja wstecz nie jest dostępna w trybie wirtualnego bufora.",
    ...
    def OnForward(self, event):
        if isinstance(self.browser, wx.html2.WebView):
            ...
            wx.MessageBox("Nawigacja do przodu nie jest dostępna w trybie wirtualnego bufora.",
                          "Informacja", wx.OK | wx.ICON_INFORMATION)
    ...
    def OnNavigating(self, event):
        ...
        self.statusbar.SetStatusText("Ładowanie strony...")
        ...
        if self.settings.getboolean('announcements', 'loading_messages'):
            threading.Thread(target=speak, args=("Ładowanie strony...",)).start()

    def OnPageLoaded(self, event):
        """Po załadowaniu strony w WebView."""
        self.statusbar.SetStatusText("Strona załadowana.")
        ...
        if self.settings.getboolean('announcements', 'loading_messages'):
            ...
            threading.Thread(target=speak, args=(f"Załadowano stronę {title}",)).start()
    ...
    def OnBrowserCharHook(self, event):
        ...
        - F6 lub Ctrl+L -> fokus na pasek adresu
        (działa również, gdy jesteśmy w dokumencie).
        """
        keycode = event.GetKeyCode()
        ctrl_down = event.ControlDown()
        ...
        if keycode == wx.WXK_F6 or (ctrl_down and keycode == ord('L')):
            self.address.SetFocus()
            self.address.SelectAll()
        elif keycode == wx.WXK_TAB and shift_down:
        ...
        for element in self.interactive_elements:
            if element['start'] <= position <= element['end']:
                if element['type'] == 'link':
                    self.address.SetValue(element['action'])
                    self.LoadVirtualBuffer(element['action'])
                elif element['type'] == 'button':
                    wx.MessageBox(f"Kliknięto przycisk: {element['label']}", "Informacja", wx.OK | wx.ICON_INFORMATION)
                return
        event.Skip()
        ...
        if isinstance(self.browser, wx.html2.WebView):
            ...
            if result:
                ...
                if len(data) == 4:
                    ...
                    message = f"Strona zawiera {forms} pól formularza, {links} łączy, {buttons} przycisków i {rows} wierszy."
    ...
    def LoadVirtualBuffer(self, url):
        ...
        def load_content():
            try:
                ...
                wx.CallAfter(self.statusbar.SetStatusText, "Strona załadowana.")
                threading.Thread(target=play_sound, args=('ding.ogg',)).start()

                if self.settings.getboolean('announcements', 'loading_messages'):
                    threading.Thread(target=speak, args=("Załadowano stronę",)).start()
                ...
                wx.CallAfter(self.statusbar.SetStatusText, "Błąd podczas ładowania strony.")
                self.loading = False
                wx.CallAfter(self.timer.Stop)
    ...
    def parse_element(self, element):
        for child in element.children:
            if isinstance(child, NavigableString):
                ...
                if child.name == 'a':
                    href = child.get('href', '')
                    link_label = child.get_text()
                    start = self.browser.GetLastPosition()
                    display_text = f"Łącze: {link_label}"
                    ...
                    button_label = child.get_text()
                    start = self.browser.GetLastPosition()
                    display_text = f"Przycisk: {button_label}"
                    self.browser.AppendText(display_text)
                    end = self.browser.GetLastPosition()
                    self.interactive_elements.append({
                        'start': start,
                        'end': end,
                        'type': 'button',
                        'action': child.get('onclick', ''),
                        'label': button_label
                    })
                elif child.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
                    self.browser.AppendText(f"\nNagłówek: {child.get_text()}\n")
                elif child.name == 'br':
...
class SettingsDialog(wx.Dialog):
    def __init__(self, parent):
        super(SettingsDialog, self).__init__(parent, title="Ustawienia tBrowser")
        self.settings = config
        ...
        # Grupa "Oznajmianie"
        announcement_box = wx.StaticBox(panel, label="Oznajmianie")
        announcement_sizer = wx.StaticBoxSizer(announcement_box, wx.VERTICAL)

        self.announce_summary_cb = wx.CheckBox(panel, label="Oznajmiaj podsumowanie strony")
        self.announce_summary_cb.SetValue(self.settings.getboolean('announcements', 'announce_page_summary'))

        self.loading_messages_cb = wx.CheckBox(panel, label="Komunikaty o ładowaniu strony")
        ...
        # Grupa "Interfejs"
        interface_box = wx.StaticBox(panel, label="Interfejs")
        interface_sizer = wx.StaticBoxSizer(interface_box, wx.VERTICAL)

        view_mode_label = wx.StaticText(panel, label="Wybierz tryb przeglądania strony:")
        self.view_mode_choice = wx.Choice(panel, 
            choices=["Widok sieciowy (edge)", "Tryb wirtualnego bufora"])
        ...
        privacy_box = wx.StaticBox(panel, label="Prywatność")
        privacy_sizer = wx.StaticBoxSizer(privacy_box, wx.VERTICAL)

        # Checkbox do blokowania komunikatów o cookies
        self.block_cookies_cb = wx.CheckBox(panel, label="Nie wyświetlaj alertów o plikach cookie (o ile to możliwe)")
        ...
        save_btn = wx.Button(panel, label="Zapisz")
        cancel_btn = wx.Button(panel, label="Anuluj")
    ...
    def OnSave(self, event):
        ...
        wx.MessageBox("Ustawienia zostały zapisane. Uruchom ponownie aplikację, aby zastosować zmiany.",
                      "Informacja", wx.OK | wx.ICON_INFORMATION)
