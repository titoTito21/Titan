...
# --------------------------------------------------------------------------- #
def _catalogue_path():
    """Where the catalogue is kept - NVDA's configuration folder, or
    Titan's own reader folder where there is no NVDA.

    It is `labels._folder()` rather than a second answer to the same
    question: the two readers keep everything else they accumulate side by
    side, and a catalogue written next to the module would be inside the
    installed add-on, where an update replaces it.
    """
    directory = ''
    try:
        from . import labels
        directory = labels._folder()
