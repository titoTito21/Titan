...
def busy_wanted():
    ...
    return switchboard.read('busyState', True)
...
def _cue(which):
    ...
    In-process where this runs inside Titan (Titan Access), over the bus
    from NVDA - `icons.play_titan` knows which; `earcons` is the add-on's
    own module and is not in the other reader.
    """
    name = {'busy': 'ellipses.ogg', 'ready': 'clicked.ogg'}.get(which, '')
    if not name:
        return
    try:
        from . import icons
        if icons.play('reader.' + which):
            return
        if icons.play_titan('reader/' + name):
            return
    except Exception:                                # noqa: BLE001
        pass
    try:
...
def attention_wanted():
    from . import switchboard
    return switchboard.read('attentionState', True)
...
def _flashed(hwnd):
    ...
    try:
        from . import icons
        if not icons.play('reader.attention') and \
                not icons.play_titan('reader/notification.ogg'):
            from . import earcons
