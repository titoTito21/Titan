...
def _announce(entered):
    ...
    **This is a mode change, and a mode change has to be announced.** The
    ...
    try:
        from . import icons
        if not icons.play('reader.review-on' if entered else 'reader.review-off'):
            from . import earcons
...
def say(moved):
    ...
    if speech is None:
        return False
    try:
        sequence = voices.sequence(parts) if voices is not None else None
        if sequence is None:
            raise ImportError('no voices here')
    except Exception:                                # noqa: BLE001
        # The other reader has no NVDA speech commands: the parts as one
        # line, which loses the tones and keeps every word.
        sequence = [', '.join(str(text) for text, _voice in parts)]
    if not sequence:
        return False
    try:
        speech.cancelSpeech()
...
def _click(rect, name, double=False):
    """Click (or double-click) the middle of a rectangle, mouse put back.
    ...
    try:
        ...
        if was is not None:
            try:
                ...
                pass
    if double:
        # Translators: said when a control read from the screen is
        # double-clicked.
        return True, _('Double-clicked {name}.').format(name=name)
