...
def build():
    ...
    class Manager(wx.Dialog):
        ...
        def _icons_page(self):
            ...
            page = wx.Panel(self.book)
            # Translators: a page of the manager window.
            self.book.AddPage(page, _('Sounds'))
            helper = guiHelper.BoxSizerHelper(page, orientation=wx.VERTICAL)
            # Translators: the list of reader events that have a sound.
            self.icons = helper.addLabeledControl(
                _('&Events'), _check_list_class())
        ...
        def _fill_icons(self, at=0):
            # **Rows are EVENTS, in words** - "the focus lands on a check
            # box", "something opened" - never the name of a sound file.
            self._icon_rows = icons.described()
            self.icons.Set([row['label'] for row in self._icon_rows])
        ...
        def _icon_chosen(self, _event):
            ...
            if row is None:
                return
            # Translators: the sounds manager: what the built-in sound for
            # this event is called. {what} is the sound's own meaning.
            said = _('Built-in sound: {what}').format(what=row['meaning'])
            if not row['file']:
