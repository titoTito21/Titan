...
def _folder():
    """Where this reader keeps it - NVDA's own folder, or Titan's.

    The same file is byte-identical in Titan Access, which has no NVDA
    under it; asking for both here is what lets it be. See
...
def all_programs():
    """Every program that has an answer of its own, for the manager."""
    return sorted(_load().keys())


def answers_for(program):
    """{switch: answer} for one program - only what it really answered."""
    return dict(_load().get(str(program or '').lower()) or {})
