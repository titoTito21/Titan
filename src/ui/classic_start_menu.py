...
class ClassicStartMenu(wx.Frame):
    ...
    def load_titan_games_submenu(self, parent_item):
        """Ładowanie gier Titan z grupowaniem według platform"""
        print(f"DEBUG: Loading Titan games submenu with platform grouping, parent_item: {parent_item}")
        try:
            # Use cache if available, otherwise load fresh
            if self._games_cache is None:
                print("DEBUG: Loading games from game_manager")
                from game_manager import get_games_by_platform
                self._games_cache = get_games_by_platform()
                print(f"DEBUG: Loaded games by platform to cache: {self._games_cache.keys()}")

            games_by_platform = self._games_cache
            ...
            platform_order = ['Steam', 'Battle.net', 'PC']
            for platform in platform_order:
                if platform not in games_by_platform or not games_by_platform[platform]:
                    continue

                games = games_by_platform[platform]
                print(f"DEBUG: Adding platform folder: {platform} with {len(games)} games")

                # Create platform folder
                platform_display = _(platform)
                platform_folder = self.menu_tree.AppendItem(parent_item, f"{platform_display} ({len(games)})")
                self.menu_tree.SetItemData(platform_folder, ClassicMenuItem(platform_display))

                # Add games under platform
                for i, game in enumerate(games):
                    print(f"DEBUG: Adding game {i+1}/{len(games)}: {game.get('name', 'Unknown')} to {platform}")
                    game_item = self.menu_tree.AppendItem(platform_folder, game['name'])
                    self.menu_tree.SetItemData(game_item, {'type': 'titan_game', 'data': game})

            print(f"DEBUG: Successfully added games by platform to submenu")
